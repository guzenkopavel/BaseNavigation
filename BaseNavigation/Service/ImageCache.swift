...
protocol ImageCacheType: AnyObject {
    // Returns the image for url
    func image(for url: URL) -> UIImage?

    // Inserts the image for url to cache
    func insertImage(_ image: UIImage?, for url: URL)

    // Removes the image by url from cache
    func removeImage(for url: URL)
}

final class ImageCache {
    // Cache with encoded images
    ...
    // Config for image cache
    struct Config {
        // Max image count
        let countLimit: Int
        // Max size of memory for caching
        let memoryLimit: Int
